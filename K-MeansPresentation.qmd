...
- K-means clustering caters to voluminous & unpredictable data
- Finds patterns within unlabeled groups
- Provides potential business insights

:::

::: notes

There is a heightened demand for more robust analytical processes as data continues growing at large rates.

Unlabeled data that may have groups or patterns suggesting diverse sub-populations within the population.

We will explore the components of k-means clustering and learn how finding these "clusters" or sub-populations can be utilized within eCommerce businesses to improve customer marketing and revenue generation.

:::

## Key Concepts

We will discuss the following foundational concepts of k-means clustering:

::: {.fragment .fade-up}
-   Unsupervised learning
-   Clustering
-   Distance measurement using the Euclidean Distance.
:::
...
-   Online retailer based in the United Kingdom
-   406,829 customer transactions
...
We will first explore ways to better summarize individual customer transactions.
...
We will use a subset of the data excluding the UK observations as to avoid skewness in the data.
:::
...
-   Removed transactions with item quantities $< 0$
...
-   Reassign all data points to the closest cluster using euclidean distance
-   Compute the mean center of each new cluster (centroid)
...
Improvements and additions have been made to the base k-means methodology to optimize performance and increase scalability

-   This data may benefit from the exploration of differences in distance formulas such as the Mahatten distance and Chebyshev distances

-   Additional $k$ selection methods such as the Empirical and Silhouette methods

-   The use of dimensionality reduction techniques to improve efficiency and overall clustering results
