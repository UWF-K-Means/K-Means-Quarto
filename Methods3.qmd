...
2.  Then compute seed points, which are randomly assigned reference points called $centroids$ @Morissette2013. In the following figures we will see $K=4$, which are highlighted in green.

3.  Assign each object to the cluster with the nearest assigned $centroid$. @fig-centroids shows the Euclidean Distance calculation as a red line connecting each observation to one of the 4 $centroids$.
