...
$μ_i$ represents the centroid mean of cluster $i$.
...
To determine k, the number of clusters to initially set, we can use the empirical method or the elbow method. The empirical method assigns the number of clusters ≈$\frac{n^{\frac{1}{2}}}{2}$ for a dataset of $n$ observations. The elbow method is based on the idea that increasing the number of clusters k will reduce the sum of within-cluster variance. After plotting the sum of within-cluster variances as a function of the number of clusters $K$, the inflection point is determined using the turning point in the curve.

The k-means algorithm for partitioning can be executed in 5 main steps.

1.  Partition $N$ objects, or observations, into $K$ nonempty subsets. Each cluster must not be empty and will have a different classification. This is $K$, if $k=2$ that means there will be two clusters.

2.  Then compute seed points, which are randomly assigned reference points called centroids.

3.  Assign each object to the cluster with the nearest assigned centroid.

4.  Adjust the centroid location using the Euclidean distance formula, above, to minimize the distance of each data observation to its assigned centroid.

5.  The final step is to repeat steps 2-4 until there is no change in distance between the observations and the centroids.

#Visual representation with notation OR use steps above and place appropriate pictures above/under the corresponding step.
